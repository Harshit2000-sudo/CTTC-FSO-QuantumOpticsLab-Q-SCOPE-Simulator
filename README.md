...
1. **Clone the Repository**
   ```bash
   git clone https://github.com/W1Adri/QKD-satellite-link-simulator.git
   cd QKD-satellite-link-simulator
   ```
...
The BB84 protocol models a weak coherent pulse (WCP) source with mean photon number μ = 0.5.
The total detection rate includes both signal and noise clicks (dark counts + background).
QBER is computed as the ratio of error clicks to the **total** detection rate:

$$\text{QBER} = \frac{R_\text{noise}/2}{R_\text{sig} + R_\text{noise}}$$

where $R_\text{noise} = R_\text{dark} + R_\text{bg}\,\eta_\text{det}$ is the total noise click rate
and the factor 1/2 reflects that random noise clicks are wrong half the time.
The secure key rate uses the simplified Shor-Preskill bound:

$$R_\text{sec} = R_\text{sift}\bigl[1 - h(Q) - f_\text{EC}\,h(Q)\bigr]$$

and is set to zero above the 11% QBER threshold.

#### BB84 Decoy-State Protocol (recommended)

Implements the two-intensity decoy-state QKD protocol from Ma et al. (2005) with the
GLLP/Shor-Preskill security bound. This is the scientifically preferred estimator for
satellite QKD because it correctly accounts for multi-photon pulses that simple BB84 ignores.

**Physical model:**
- Signal intensity μ_s (default 0.6) and decoy intensity μ_d (default 0.1)
- Exact threshold-detector gain model: $Q_\mu = 1-(1-Y_0)\,e^{-\mu\,\eta_\text{tot}}$
- Single-photon yield lower bound from decoy analysis
- Phase error upper bound from decoy QBER
- GLLP key rate: $R \ge q\{\mu_s e^{-\mu_s} Y_1^\text{low}[1-h(e_1^\text{up})] - Q_s f_\text{EC} h(E_s)\}$

**Finite-key correction (optional):**
For a satellite pass of duration T, the total sifted bits n_sifted may be only 10⁴–10⁶.
Statistical fluctuations reduce the extractable key. Pass the parameter `finite_key_n`
(estimated sifted bits) and `epsilon_sec` (security parameter, default 10⁻¹⁰) to get
the finite-key corrected rate:

$$R_\text{finite} = r_\text{fk} \times R_\infty, \quad r_\text{fk} = \max\!\left(0,\,1 - \frac{\sqrt{-8\ln\epsilon}}{\sqrt{n_\text{sift}}}\right)$$

**Route in API:** `"protocol": "bb84-decoy"`

**Extra output fields:**
- `singlePhotonYield` – lower bound on $Y_1$
- `singlePhotonPhaseError` – upper bound on $e_1$ [%]
- `signalGain`, `decoyGain` – $Q_s$, $Q_d$
- `finiteKeyFraction` – $r_\text{fk}$ (1.0 if no finite-key correction requested)
...
5. **QKD Protocol Details**
   - BB84 and decoy-state BB84 estimators implemented; finite-key is a first-order approximation
   - Full composable security with Lim et al. (2014) fluctuation bounds is a planned improvement
   - E91 and CV-QKD are qualitative estimators only
...
Copyright © 2026 Francesc Adrià Sancho González, Centre Tecnològic de
